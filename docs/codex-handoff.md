...
- **Regression coverage:** Added transition-retry tests for canonical persistence, website publication, and email failure. A retry preserves one canonical run and skips already-complete destinations. The first guarded contract build caught two incorrect test expectations for already-persisted delivery failures; the assertions were corrected before retrying deployment.

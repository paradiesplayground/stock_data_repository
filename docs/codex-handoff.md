...
- **Current status:** Commit `15f6d9b` is pushed to `main`; deployment remains pending. The Daily Stock Alert automation remains disabled until the deployed MCP manifest is refreshed and verified with the read-only annotations; no August 28 validation, persistence, website publication, SMTP delivery, or mailbox verification occurred during this fix.

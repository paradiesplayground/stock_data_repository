...
- **Regression coverage:** Added factual KLIC-style presentation, high-R/low-score explanation, Markdown canonical-field, rounded trigger-distance, and research-scope-only downgrade tests. Local Python tests remain unavailable because the checked-in virtualenv points at a removed interpreter; deployment contract validation is pending.
